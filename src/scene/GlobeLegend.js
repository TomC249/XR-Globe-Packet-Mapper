...
// Heat scale for compact traffic clumps — colour by source strand count
...
export class GlobeLegend {
  ...
  init(sceneRoot, globeRoot, globeRadius) {
    ...
    // No parent — world position is driven manually every frame so no rotation
  ...
  /**
   * Call once on XR entry with the user's normalised horizontal facing direction.
   * Stores the base world-space offset and snaps the initial panel orientation.
   */
  reposition(forward) {
    if (!this.#plane || !this.#sceneRoot) return;
    const r = this.#globeRadius;

    // Babylon is left-handed Y-up: right = up × forward = (fz, 0, −fx).
    const right = new Vector3(forward.z, 0, -forward.x);

    // Base offset at scale 1 — to the user's right of globe centre,
    // slightly pulled towards the user (less than before so it sits closer).
    ...
    // Babylon's CreatePlane front-face normal is −Z (not +Z).
    // To show the front face towards the user (who is in the −forward direction
    // from the globe), we need local −Z to point in the −forward direction,
    // which means rotation.y = atan2(forward.x, forward.z).
    this.#plane.rotation.y = Math.atan2(forward.x, forward.z);
  }

  /**
   * Call every render frame while in XR.
   * Tracks the globe centre (with scale) and applies a soft Y-axis billboard:
   * barely rotates at the periphery, tracks responsively when faced directly.
   */
  update(camera) {
    if (!this.#plane?.isEnabled() || !camera || !this.#sceneRoot) return;

    // ── 1. Follow globe centre, scaled with globe zoom ────────────────────────
    const scale  = this.#globeRoot?.scaling.x ?? 1;
    const centre = this.#sceneRoot.getAbsolutePosition();
    this.#plane.position.x = centre.x + this.#worldOffset.x * scale;
    this.#plane.position.y = centre.y + this.#worldOffset.y * scale;
    ...
    // ── 2. Soft billboard (Y-axis) ────────────────────────────────────────────
    const toCam = camera.position.subtract(this.#plane.position);
    toCam.y = 0;
    if (toCam.length() < 0.001) return;
    toCam.normalize();

    // Front-face normal in world space is local −Z:
    //   panelFront = (−sin(curY), 0, −cos(curY))
    // Target: make panelFront align with toCam →
    //   −sin(r) = toCam.x  →  r = atan2(−toCam.x, −toCam.z)
    ...
    // Cubic falloff: barely moves at sides, snappy at centre.
